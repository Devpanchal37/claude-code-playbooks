...
3. Real-time / event issues (room not joined, event name mismatch, socket disconnect)
4. Storage issues (wrong box name, key mismatch, box not opened)
5. Navigation issues (wrong arguments, missing result handling)
6. Form / Lifecycle / Async / Pagination issues

> **NOTE:** See the `[Flutter-GetX Specifics]` section below for the exact hypothesis patterns when using GetX. Generic Flutter projects should substitute their state management's equivalent patterns.
...
| State not updating after action | "After accepting a request, the card is still visible in the list" | FULL INVESTIGATION | 4-step gate sequence, Phase 2 hypothesis: state management list not refreshed | Handoff Brief, CONFIRMED |
...
A plan that reverses this order (presentation first, then domain) produces a development experience where the developer is constantly blocked waiting for dependencies to exist.
...
| Shared file modification without blast-radius analysis | Changing `rest_apis.dart` or a shared entity touches every module that imports it | Read `docs/maps/project_map.md` blast-radius matrix for that file before finalizing |
...
### Store to Claude-Mem (After Plan Confirmed)

After human confirmation, store non-obvious architectural decisions to claude-mem:
...
- An existing pattern was intentionally NOT followed (and why matters)
- A cross-module integration approach was decided that affects future features
...
## Reuse Identified
- Existing API functions: [list from api_registry.md — or "none"]
...
|----------|----------|--------|-------|
| getItems() | /api/v1/items | GET | New — must be added |
| getUserProfile() | /api/v1/profile | GET | Reuse existing — do NOT re-create |
...
- lib/src/features/[name]/presentation/widgets/[name]_card.dart

### Phase 4 — Registration and Wiring
- Route constant added to routes file
- Binding registered with DI
- Launched from: [which screen or navigation entry point]
- Cross-screen events: [event names and which controllers consume them]

## 4 UI States Per Screen
### [ScreenName]
- Loading: shimmer skeleton (list) or progress indicator (form)
- Error: error widget + retry button + error message locale key
- Empty: empty state widget + call-to-action locale key
- Success: [describe the success layout]

## Business Rules to Implement
- [ ] [specific business rule from FR]
- [ ] [specific validation rule]

## Risks & Decisions
- Risk: [potential issue] → Mitigation: [how to handle]
- Decision needed: [ambiguity in FR that human must resolve before developer starts]

## Out of Scope (Developer must NOT add)
- [anything not in the FR that a developer might add "helpfully"]
...
**Symptom:** Phase 1 (ui-reviewer) completed. Phase 2 never started. Loop-operator restarts the pipeline from Phase 1. ui-reviewer runs again. All Phase 1 work is repeated. The original Phase 1 findings may differ from the restart's findings.
...
**Cause:** STALL/WAIT classification step was skipped. Loop-operator went directly to intervention.
