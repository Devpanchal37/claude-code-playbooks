...
The trigger classification and routing rules above apply to all Flutter projects. The GetX + Clean Architecture stack adds specificity in three areas: quality tier classification signals, sensitive surface detection, and developer launch context.

### GetX-Specific Quality Tier Classification

When classifying post-implementation tier for a GetX project, use these precise signals in addition to the generic tier rules above:

**Tier 1 signals (flutter analyze only, no reviewer):**
- `Text()`, `Icon()`, `Container()` property-only changes
- Widget `Visibility` or conditional rendering with `isVisible` variable
- `maxLength`, `maxLines`, padding/margin values changed
- Label or placeholder text updated via locale key

**Tier 2 signals (code-reviewer required):**
- `Obx()` reactive widget added, removed, or its observable references changed
- `GetX<ProfileController>` widget or `Get.find<>()` call path modified
- `.obs` variable added or renamed on a controller
- `.assignAll()` list mutation patterns changed
- `Get.toNamed()` navigation arguments modified
- `Get.arguments` or `Get.parameters` access paths changed
- Hive `Box.put()` or `Box.get()` calls added or modified
- Socket.IO `socket.emit()` payload structure or `socket.on()` event name changed
- `Get.lazyPut()` or `Get.put()` registration in a Binding modified

**Tier 3 signals (full quality loop):**
- New `.dart` file created in `presentation/screens/` or `presentation/widgets/`
- New `Controller` class created (extends `GetxController`)
- New `Binding` class created (extends `Bindings`)
- New function added to a repository class
- `AppRoutes` or `AppPages` modified (new route registered)
- Shared model class in `domain/entities/` modified

### GetX Sensitive Surface Detection

security-reviewer fires (in addition to the generic sensitive surface list) when ANY of these are added or modified:

- Auth token Hive storage: `Hive.box('sessionBox').put('token', ...)` or equivalent
- API request headers with bearer tokens: `'Authorization': 'Bearer ${token}'`
- Socket.IO authentication handshake: `socket.auth`, `socket.connect()` with credentials object
- User input flowing to an API call without a visible validation step in the controller
- `Get.put()` registration of a controller that manages session state or user identity
- Any controller that stores user identity fields (userId, email, token) as `.obs` variables

### GetX Developer Launch Context

For GetX Clean Architecture projects, the developer agent benefits from receiving the architecture layer being implemented in its launch prompt. Main Claude adds this when launching developer after planner or FR docs review:

```
Task: Implement [FeatureName] module
Plan: [implementation plan or FR file path]
Architecture phase: [models | repositories | controllers | screens | bindings | routes]
```

This prevents the developer from jumping to screen implementation before domain and data layers exist — a common failure mode in GetX Clean Architecture where a controller references a repository method that has not been created yet.

Include the following in agents.md for GetX projects:

```markdown
## Developer Launch Context (GetX Projects)

When launching developer for a new GetX module, Main Claude adds architecture phase context:
- Phase sequence: models → repositories → controllers → screens → bindings → routes
- Each phase must complete before the next begins
- Include the current phase in the developer launch prompt to prevent layer ordering violations
```
