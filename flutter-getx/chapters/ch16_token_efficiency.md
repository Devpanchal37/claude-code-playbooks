...
The GetX + Clean Architecture stack has several high-frequency files that are easy to over-read. The one-read rule applies to these files with particular force.

### High-Traffic Files in a GetX Project

These files are referenced often and must never be re-read within a session once loaded:

| File | Why it's expensive to re-read | One-read rule |
|------|------------------------------|---------------|
| `lib/src/core/routes/app_pages.dart` | Referenced every time a route is added. Same content each time. | Read once when routes need to be checked. Cache the route list. |
| `lib/src/locale/languages_en.dart` | Referenced every time a new locale key is added. | Read once per session, cache the key list. Do not re-read to verify a key exists — grep instead. |
| `lib/src/theme/color_helper.dart` | Referenced every time a color is used in a widget. | Read once per session. Apply from memory. Do not re-read to check a color name — grep the class. |
| `docs/instructions/ARCHITECTURE.md` | Defines GetX layer rules, file naming, Clean Architecture enforcement. | Read once at session start. Same rules apply for every feature. |

### GetX-Specific Reading Hierarchy for Common Queries

| Question | Cheapest tool |
|----------|--------------|
| Does a locale key `locale.profileName` exist? | Grep `languages_en.dart` for `profileName` |
| Does a route `/profile-detail` exist in app_pages? | Grep `app_pages.dart` for `profile-detail` |
| Which modules use `UserEntity`? | `get_impact_radius_tool` or `project_map.md` Entity Dependency Table |
| Does a `ProfileController` binding exist? | Grep `lib/` for `ProfileController` in a Binding class |
| What is the correct GetX controller disposal pattern? | Apply from cached ARCHITECTURE.md — do not re-read |

### GetX Controller Type in the Project Map

The project map's Blast Radius Matrix should classify GetX controllers by disposal type — because changing a permanent controller to non-permanent (or vice versa) breaks every screen that relies on it being in memory.

When reading `project_map.md` for impact analysis on a GetX controller change, check the controller's entry for its disposal classification. If the project map does not have this classification — add it. One additional column: `Controller Type: permanent / non-permanent`.

This single column prevents the debugger from having to read `app_pages.dart` and all binding files to determine whether a controller disposal change will break other screens.
...
| GetX: one-read files | `app_pages.dart`, `languages_en.dart`, `color_helper.dart` |
