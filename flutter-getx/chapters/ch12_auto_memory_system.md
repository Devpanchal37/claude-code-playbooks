...
GetX introduces lifecycle and reactivity patterns with specific failure modes that are not obvious from documentation. These are exactly the kind of corrections that belong in auto-memory — they prevent agents from re-discovering the same GetX gotcha every session.

### Feedback Memory: GetX Controller Lifecycle with IndexedStack

The most common GetX lifecycle mistake: using `Get.put()` for controllers that serve tab-based navigation without `permanent: true`. GetX auto-disposes non-permanent controllers when the registered route leaves the stack. For an `IndexedStack` tab layout, the controller is never "popped" in the navigation sense — but GetX still disposes it on route changes, causing state to reset silently on every tab switch.

```markdown
---
name: GetX permanent controllers for IndexedStack tabs
description: IndexedStack tab controllers must use Get.put(permanent: true) — GetX disposes non-permanent controllers on route change
type: feedback
---

Always use `Get.put(Controller(), permanent: true)` for any controller bound to an
IndexedStack tab view.

**Why:** IndexedStack tabs are never popped in GetX's route sense, but GetX still
auto-disposes non-permanent controllers when the registered route context changes.
Without permanent: true, the controller rebuilds on every tab switch, silently resetting
all state. This is a non-obvious difference from push/pop navigation where disposal
behavior is predictable.

**How to apply:** Any controller that is registered in a Binding for an IndexedStack
child widget must use Get.put(..., permanent: true). Only push-and-pop route controllers
(detail screens, modals, chat room screens) should be non-permanent.
```

### Feedback Memory: Obx Reactivity Scope

Another common GetX mistake: wrapping a parent widget in `Obx` and expecting child widget classes to also be reactive. Only the widget tree returned directly by `Obx`'s builder closure is reactive. Separate widget classes that read observables must have their own `Obx` — they do not inherit reactivity from a parent `Obx`.

```markdown
---
name: Obx reactivity scope
description: Obx only makes its direct return reactive — child widget classes are not reactive unless they have their own Obx
type: feedback
---

Obx reactivity applies only to the widget tree returned directly in its closure. A
child widget class that reads a controller's observable does not become reactive simply
because its parent is wrapped in Obx.

**Why:** Obx registers observables read during its own build execution. If an observable
is read in a separate child widget's build() method, that widget subscribes independently.
It will only rebuild if it has its own Obx — the parent's Obx firing does not trigger child
widget rebuilds.

**How to apply:** Any widget class that reads an observable (controller.x.value,
controller.items.length) and must rebuild when that observable changes needs its own
Obx wrapper. Do not assume that a parent-level Obx propagates reactivity to extracted
widget classes.
```

### Project Memory: Architecture Decision — Controller Scoping

When an architecture decision is made during development that isn't formally captured in `ARCHITECTURE.md`, a project memory prevents agents from re-proposing the rejected approach in future sessions.

```markdown
---
name: No global singleton controllers
description: Architecture decision — all GetX controllers scoped to their feature binding, no app-level Get.put() singletons
type: project
---

All GetX controllers are scoped to their feature binding and registered only when
that feature's route is active. No controller is registered as a global singleton
with Get.put() at app startup, with the single exception of AppController which manages
authentication state.

**Why:** Global singletons accumulate state across the app lifetime, making it impossible
to reset feature state when re-navigating. Each feature must start in a clean state when
its route is entered. This also prevents memory leaks from controllers that hold network
subscriptions or stream listeners.

**How to apply:** When the planner or developer agent proposes registering a controller,
verify it uses a Binding class scoped to its route, not a global Get.put() at app startup.
AppController is the only permitted exception — flag any other proposal that uses
Get.put() outside of a Binding.
```
