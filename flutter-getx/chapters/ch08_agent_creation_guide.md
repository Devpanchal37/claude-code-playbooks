...
### GetX-Specific Classification in Debugging Agents

When designing a debugging agent for a GetX project, the classification gate must cover GetX-specific bug categories that are distinct from generic Flutter bugs. These require different investigation paths.

**Extended classification table for GetX projects:**

| Input type | Classification | Investigation path |
|------------|---------------|-------------------|
| Wrong value / count / amount | DATA GATE | Ask for API log first — never read source |
| `Get.find<ControllerName>()` throws "not found" | GETX_DI_BUG | Check Bindings file — is controller registered? Check `Get.put` vs `Get.lazyPut` |
| Obx widget not updating when observable changes | GETX_REACTIVE_BUG | Check: is the variable `.obs`? Is Obx accessing it directly or through a non-reactive wrapper? |
| Controller disposed mid-session | GETX_LIFECYCLE_BUG | Check: is this a permanent controller on an IndexedStack tab? Should use `Get.put(..., permanent: true)` |
| Navigation result not reflected (pop-and-back) | GETX_NAVIGATION_BUG | Check `Get.back(result: ...)` and `Get.toNamed(...).then(...)` pattern |
| RxList not updating list widget | GETX_RXLIST_BUG | Check: is `.assignAll()` used for full replacement, not `=`? |
| Socket event received but state not updated | SOCKET_STATE_BUG | Check: controller is listening? Observable is `.obs`? Obx is wrapping the widget? |

### GetX-Specific Developer Agent Compliance Greps

When designing the developer agent for a GetX project, the compliance greps section must include GetX-specific checks that run after every widget file is written:

```bash
# Required additional checks for GetX projects

# Check 1: No permanent controller used outside IndexedStack tabs
grep -n "Get.put.*permanent: true" lib/src/features/ --include="*.dart"
# Expected: Only DashboardBinding or equivalent tab controller binding

# Check 2: No controller accessed without Get.find inside a stateless widget
grep -n "Get.find<" lib/src/features/ --include="*.dart"
# Verify each result: is the Get.find inside an Obx or in a controller's method?

# Check 3: RxList full-replacement uses .assignAll(), not =
grep -n "\.value = \[" lib/src/features/ --include="*.dart"
# This pattern replaces an observable — should use .assignAll([...]) instead

# Check 4: No Colors.xxx or Color(0xFF) — use ColorHelper
grep -n "Colors\.\|Color(0x" lib/src/features/ --include="*.dart"
# Expected: empty

# Check 5: Obx wraps only the minimal reactive subtree
grep -n "Obx(" lib/src/features/ --include="*.dart"
# Review each: does it access .obs variables in its own closure, not deep in a child?
```

Add these as explicit compliance greps in the developer agent's "Widget Placement Gate" section.

### GetX Controller Lifecycle — Agent Design Requirement

Any agent that touches controller registration (developer agent, code-reviewer) must encode this GetX lifecycle rule:

```markdown
## GetX Controller Lifetime Rule (MANDATORY CHECK)

Before writing any Get.put() or registering in a Binding:

1. Is this controller for an IndexedStack tab (Dashboard, MainMenu)?
   → MUST use: Get.put(ControllerName(), permanent: true)
   → Reason: IndexedStack tabs are never popped — GetX auto-dispose would destroy
     the controller mid-session, making the tab unresponsive

2. Is this controller for a push-and-pop route (ProfileScreen, ChatRoomScreen)?
   → MUST use: Get.put(ControllerName()) [NOT permanent]
   → Register in the route's Binding, not in a global binding
   → Reason: controller should be disposed when the route is popped (memory management)

3. Does this controller need to be found by another controller (Get.find)?
   → Verify the referenced controller is registered in a parent scope that
     is guaranteed to be alive when the Get.find runs
```

### GetX-Specific Protected Files

Add these GetX files to the Protected Files section of every agent that writes to `lib/`:

| File | Owner | Why protected |
|------|-------|--------------|
| `lib/src/core/routes/app_routes.dart` | developer agent | Route constants — adding a constant is the only allowed change. No removal without impact check. |
| `lib/src/core/routes/app_pages.dart` | developer agent | Route-to-screen mapping — every new screen must be registered exactly once |
| `DashboardBinding` (or equivalent tab binding) | developer agent | Tab controller registrations — every permanent controller must appear here |
| `lib/src/core/network/api_service.dart` | developer agent | Central HTTP client — changes affect all API calls across all modules |

These files have high blast radius — changes affect every module. They should only be touched as the final step of a feature implementation, after all other files are complete and verified.

### GetX-Specific Agent Description Examples

The `description` field in the frontmatter is the routing signal. For GetX projects, the descriptions must be precise enough to distinguish GetX-specific work from generic Flutter work:

```yaml
# Developer agent — GetX project
description: >
  Complete Flutter/GetX development workflow executor. Follows CLAUDE.md 
  "Develop [module]" protocol exactly. Handles module intake, per-feature 
  implementation loops with Clean Architecture (domain → data → presentation), 
  GetX controller/binding registration, checkpointing, self-validation, and 
  documentation updates.

# Systematic debugger — GetX project  
description: >
  Flutter/GetX systematic debugging specialist. CRITICAL IDENTITY: For ANY bug 
  where a value, count, or data field is wrong/mismatched — the DATA GATE fires 
  IMMEDIATELY before any file reading. Classifies all other inputs — GetX state 
  bugs, controller lifecycle bugs, Obx reactivity bugs, socket event bugs — go 
  to FULL INVESTIGATION. Do NOT use for build/compile errors.

# Code-reviewer — GetX project
description: >
  Flutter/GetX code review specialist. Reviews for Clean Architecture violations 
  (logic in build(), wrong controller lifetime, missing Obx), UI compliance 
  (ColorHelper, locale keys, shimmer), and GetX patterns (permanent controllers, 
  RxList mutation, Binding registration). Reviews only changed files unless 
  impact analysis requires broader scope.
```

Note how each description names the specific violation types the agent checks — this makes routing from Main Claude deterministic rather than fuzzy.
...
| GetX compliance greps | Section [Flutter-GetX Specifics] above |
| GetX controller lifetime rule | Section [Flutter-GetX Specifics] above |
