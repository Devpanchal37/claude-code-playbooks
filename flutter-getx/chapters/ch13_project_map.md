...
GetX introduces several files with unusually high blast radius that must be in your project map from day one. These are patterns that don't exist in Bloc or Riverpod projects — they are specific to how GetX handles routing, dependency injection, and state.

### GetX-Specific Tier 1 Files

These three files are Tier 1 in every GetX project regardless of size.

| File | Why Tier 1 |
|------|-----------|
| `lib/src/app/controllers/app_controller.dart` | `AppController` holds authentication state and the current user object. Every guarded screen reads from it. A field rename (e.g., `currentUser` → `user`) breaks every screen that reads the user's data for display. |
| `lib/src/app/routes/app_pages.dart` | All `GetPage` route definitions live here. Adding or renaming a `GetPage` here breaks every `Get.toNamed()` call that references the old name — but only at runtime, not at compile time. |
| `lib/src/app/routes/app_routes.dart` | All route name constants (e.g., `static const profile = '/profile'`). Every `Get.toNamed(AppRoutes.profile)` call depends on this file. A constant rename breaks navigation silently — the app compiles but the route isn't found at runtime. |

> **CRITICAL:** AppPages and AppRoutes renames cause **silent runtime failures** — not compile errors. The Dart compiler does not verify that a `String` passed to `Get.toNamed()` matches a registered route. Always list route renames as "Dangerous Changes → route not found at runtime" in the map entry.

Sample map entry for `app_routes.dart`:

```markdown
## app_routes.dart
**Path:** `lib/src/app/routes/app_routes.dart`
**Type:** GetX Route Registry — all named route constants
**Blast Radius Tier:** Tier 1 (entire app affected)

### Dangerous Changes
- Renaming any route constant → any Get.toNamed(AppRoutes.oldName) call silently fails
  at runtime (not a compile error). All 14 navigation call sites must be updated.
- Removing a route constant → same silent runtime failure pattern.

### Safe Changes
- Adding a new route constant (no existing callers affected)
```

### GetX-Specific Tier 2 Files

| File | Why Tier 2 |
|------|-----------|
| `lib/src/shared/entities/user_entity.dart` | `UserEntity` is used by the profile module (edit form), dashboard module (card display), chat module (participant names and photos), and requests module (request card). A field rename breaks 4 modules. |
| `lib/src/shared/repositories/auth_repository.dart` | Shared auth logic consumed by the auth module and the `AppController`. Changes to method signatures break both. |

### GetX Binding Blast Radius

GetX Bindings are a special category in the project map that has no equivalent in Bloc or Riverpod. A `Binding` class registers controllers with one of two lifetime modes:

- `Get.put(MyController(), permanent: true)` — controller lives for the app's lifetime
- `Get.lazyPut(() => MyController())` — controller is created on first access, disposed when the route is popped

**Why the lifetime mode is a blast-radius concern:** If a controller is registered with `lazyPut` in a tab-based `IndexedStack`, it will be disposed when a nested route pops. When the user returns to the tab, `Get.find<MyController>()` throws a "not found" error on every screen in that tab.

The project map entry for every `Binding` file must document the lifetime mode of each registered controller:

```markdown
## dashboard_binding.dart
**Path:** `lib/src/features/dashboard/bindings/dashboard_binding.dart`
**Type:** GetX Binding — registers tab controllers for IndexedStack
**Blast Radius Tier:** Tier 2

### Registered Controllers
| Controller | Mode | Scope | Why |
|------------|------|-------|-----|
| DashboardController | permanent: true | App lifetime | IndexedStack tab — must survive route pops |
| DiscoverController | permanent: true | App lifetime | IndexedStack tab — must survive route pops |
| ProfileController | permanent: true | App lifetime | IndexedStack tab — must survive route pops |

### Dangerous Changes
- Changing any controller from permanent: true → lazyPut: the tab loses its controller
  on the first nested navigation. Get.find<ThatController>() throws on return.
- Adding a new IndexedStack tab controller without permanent: true → same crash.
- Removing a controller registration → any Get.find<ThatController>() in that tab throws immediately.

### Safe Changes
- Adding a new non-tab controller with lazyPut (push-and-pop route controllers)
- Adding a new permanent controller for a new tab
```

### Why the Project-Map Agent Runs After Every Feature (GetX-Specific Reason)

In a generic Flutter project, the project-map agent runs after Tier 3 changes that touch shared files. In a GetX project, it runs after **every feature** — even ones that add no new shared entities.

The reason: every new feature adds a `Binding` file that registers controllers. A controller registered with the wrong lifetime mode (`lazyPut` when `permanent: true` is required) is a cross-module blast-radius issue that will not manifest until that route is navigated away from and back. The compiler won't catch it. Tests won't catch it unless they explicitly test navigation round-trips.

The project-map agent, after every GetX feature, verifies:

1. The new `Binding` file is added to the map with all registered controllers listed
2. Every tab controller in an `IndexedStack` is `permanent: true`
3. Every push-and-pop route controller is `lazyPut` (not `permanent: true` — that causes memory leaks)
4. If the feature adds a new tab to an existing `IndexedStack` → the parent `Binding` entry is updated

This is the GetX-specific reason why project-map is triggered for all Tier 3 changes, not only shared entity changes.

### GetX Shared File List (Starting Point)

For a feature-first GetX project, the shared file list in the agent should include at minimum:

```markdown
## Shared Files This Agent Maps

lib/src/app/controllers/app_controller.dart      → Auth state + current user (Tier 1)
lib/src/app/routes/app_pages.dart                → GetPage route definitions (Tier 1)
lib/src/app/routes/app_routes.dart               → Route name constants (Tier 1)
lib/src/core/services/api_service.dart           → HTTP client for all repositories (Tier 1)
lib/src/core/helpers/color_helper.dart           → Design tokens — all widgets (Tier 1)
lib/src/shared/entities/user_entity.dart         → User data shape — 4+ modules (Tier 2)
lib/src/core/utils/app_constants.dart             → App-wide constants (Tier 2)
lib/src/features/*/bindings/*_binding.dart       → GetX controller registrations (Tier 2–3)
lib/src/shared/widgets/*.dart                    → Shared UI components (Tier 2–3)
```

Add feature-specific shared files as they are created.
...
| Tier 1 | Entire app — HTTP client, auth session, design tokens, env config, AppController, AppRoutes |
| Tier 2 | Multiple modules — shared entities, cross-module API functions, GetX Bindings |
| Tier 3 | Single module — module-level base repository, feature-specific model |
| Tier 4 | File-local — no entry in the map |
| GetX rule | All IndexedStack tab controllers must be `permanent: true` — map binding entries enforce this |
