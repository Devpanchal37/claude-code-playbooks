...
A GetX + Clean Architecture project has a substantial set of Dart-specific rules worth encoding in `dart/coding-style.md`. These rules are what prevent agents from accidentally using the wrong state management pattern, wrong navigation method, or wrong constant source.

### What Belongs in `dart/coding-style.md` for GetX

The following categories of rules prevent the most common GetX violations:

**State management (critical — the most commonly violated)**

```markdown
## State Management: GetX ONLY

BANNED in this project:
- setState() for feature state
- Riverpod providers
- Bloc classes

CORRECT: GetxController with .obs observables
CORRECT: StatelessWidget + Obx() for reactive UI

StatefulWidget is allowed ONLY for purely local UI state
that never needs testing (animation controllers, focus nodes).
```

**Controller lifecycle rules**

```markdown
## Controller Lifecycle (CRITICAL)

IndexedStack tab controllers MUST use permanent: true:
  Get.put(HomeController(), permanent: true)   ✅
  Get.put(HomeController())                    ❌ — auto-disposed when not on screen

Push-and-pop route controllers use default (non-permanent):
  Get.put(ProfileDetailController())           ✅

Why: IndexedStack controllers are never popped from the navigation
stack, so GetX cannot dispose them. permanent: true prevents memory
leaks and reactive state loss.
```
...
RxList full replacement MUST use .assignAll(), never direct assignment:
  myList.assignAll(newList)   ✅
  myList = newList            ❌ — breaks reactivity

RxString/RxBool/RxInt: use .value for mutation:
  isLoading.value = true      ✅
  isLoading = true.obs        ❌ — creates a new observable, breaks Obx listeners
```

**UI constant sources**

```markdown
## UI Constants (ZERO TOLERANCE)

Colors:       ColorHelper.xxx ONLY — never Color(0xFF...) or Colors.red
Text styles:  TextStyleHelper.xxx ONLY — never inline TextStyle()
Sizing:       project sizing constants ONLY — never hardcoded pixels
Strings:      locale.xxx ONLY — never inline string literals in widgets
Font family:  'Plus Jakarta Sans' ONLY — never specify another fontFamily
```

**Navigation**

```markdown
## Navigation: Routes Constants ONLY

Get.toNamed(Routes.home)       ✅
Get.toNamed('/home')           ❌ — hardcoded string

All route paths are defined as static const in the routes file.
Never construct route strings manually.
```

### What Belongs in `dart/testing.md` for GetX

```markdown
## GetX Test Requirements

Always call Get.reset() in tearDown — controllers leak between tests without it.
Use GetMaterialApp (not MaterialApp) in widget tests that use Get.find().
Use ever(controller.observable, callback) to track observable changes over time.
Inject dependencies manually in tests — never rely on AppPages Bindings.

## Coverage Targets by Layer

domain/use_cases/       100% (pure Dart, no framework dependencies)
domain/entities/        100% (test copyWith, equality)
data/repositories/      80%+ (mock datasource)
presentation/controllers/ 80%+ (mock use cases)
presentation/screens/   60%+ (widget tests for key interactions)
```

### The `Obx` Scope Rule (Commonly Misunderstood)

This rule belongs in `dart/coding-style.md` because it's the source of recurring "my UI isn't reactive" bugs:

```markdown
## Obx Scope Rule

Obx reads observables ONLY within its own closure.
Child widgets extracted from the Obx closure are NOT reactive.

WRONG — child widget does not rebuild when isLoading changes:
  Obx(() => ProfileCard(isLoading: controller.isLoading.value))

CORRECT — keep the observable read inside the Obx closure:
  Obx(() {
    if (controller.isLoading.value) return const LoadingWidget();
    return ProfileCard(user: controller.user.value);
  })

Rule: if a widget needs to react to an observable, the Obx must
wrap that widget's build, not just pass the value as a parameter.
```

### Surface Critical GetX Rules in CLAUDE.md

After creating these rules in `dart/coding-style.md`, add the summary table to CLAUDE.md under "UI Compliance — Zero Tolerance":
...
| Violation | Rule |
|-----------|------|
| `Color(0xFF...)` anywhere | → use `ColorHelper.xxx` |
| `Colors.xxx` anywhere | → use `ColorHelper.xxx` |
| String literals in widget files | → use `locale.xxx` |
| `fontFamily` other than 'Plus Jakarta Sans' | → fix immediately |
| `Get.put(TabController())` without `permanent: true` | → add permanent: true |
| `myList = newList` on an RxList | → use `myList.assignAll(newList)` |
```

CLAUDE.md holds the one-liners. `dart/coding-style.md` holds the examples, rationale, and correct code patterns.
...
| GetX: key rules to encode | Controller lifecycle, observable mutation, Obx scope, UI constants |
